      ...
      <div class="col-12 col-md-6 mb-3" v-if="todayVideo">
        <div class="main-video">
          <a href="https://www.kcm.org/watch/tv-broadcast">
            <img
              :src="todayVideo.video.videoPreview"
          ...
          <div class="video-description mt-3">
            ...
            <p class="text-wrapper-3"  v-html="todayVideo.body"></p>
...
import type { BVOV_Results, BVOVItem, OtherVideos, TodayVideo } from '~/server/api/BVOVDrupal';



// Fetch the BVOV data from the API
const { data: BVOV_Data } = useFetch<BVOV_Results>("/api/BVOVDrupal");


// Function to convert UTC timestamp to local time and return a Date object
function convertToLocalTime(utcTimestamp: number): Date {
  // Create a new Date object from the UTC timestamp (in milliseconds)
...
const currentDay = currentDate.getDay(); // Get current local day (0 = Sunday, 6 = Saturday)
...
BVOVData?.items?.forEach((video: BVOVItem) => {
  ...
  const videoDate = convertToLocalTime(video.airDate); // Convert airDate to local time

  
  const isFridayVideo = videoDate.getDay() === 5; // Check if video is Friday's video

  // Logic to decide if the video is today's video or should go into otherVideos
  if ((currentDay === 6 || currentDay === 0) && isFridayVideo) {
    // On Saturday or Sunday, show Friday's video as today's video
    todayVideo = { ...video };
  } else if (currentDay === videoDate.getDay()) {
    // On other days, if current day matches the video date, it's today's video
    todayVideo = { ...video };
  } else {
    // Otherwise, add the video to the list of other videos
